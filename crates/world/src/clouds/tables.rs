...
/// `255 - 255 * 0.96^(i * 0.6015625)`, as the client's bytes rather than recomputed through `pow`.
#[rustfmt::skip]
pub(super) const COVERAGE_CURVE: [u8; 256] = [
...
/// 256 lattice gradients in `[-1, 1]` from the C runtime's `rand` under its default seed. The
/// client seeds it from the process, so any uniform table is as faithful; this one is stable.
pub(super) fn gradient_table() -> [f32; 256] {
    let mut seed: u32 = 1;
    std::array::from_fn(|_| {
...
pub(super) fn fade_table() -> [f32; 256] {
