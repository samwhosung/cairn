...
fn claim(conn: u32, seq: u32, at_ms: u32, ack: u32, movement: Movement) -> Stamped {
    Stamped {
        conn,
        seq,
        at_ms,
...
/// A crowd walking in a ring, some claiming several times a tick, a few teleporting once and
/// then taking their correction: every tick's inputs, in each connection's order.
...
fn hashes(threads: usize, order: Order, inputs: &[Vec<Stamped>]) -> (Vec<u64>, u32) {
...
fn the_same_inputs_give_the_same_world_on_one_thread_and_on_four() {
    ...
    let (one, refused) = hashes(1, Order::Canonical, &inputs);
    let (four, _) = hashes(4, Order::Canonical, &inputs);
    assert_eq!(one, four);
...
fn inputs_applied_out_of_order_change_the_world() {
    let inputs = crowd_inputs(60, 120);
    let (canonical, _) = hashes(1, Order::Canonical, &inputs);
    let (racy, _) = hashes(4, Order::Racy, &inputs);
    assert_ne!(canonical.last(), racy.last());
}

/// A batch record, reduced to what these tests look at.
...
/// The records of tick `tick`'s batch, the next message waiting on `rx`.
fn batch(rx: &mut UnboundedReceiver<Vec<u8>>, tick: u32) -> Vec<Got> {
...
fn an_observer_sees_the_near_at_once_the_far_slowly_and_never_a_refused_claim() {
    ...
    let mut run = |inputs: Vec<Stamped>| {
        pool.install(|| sim.tick(&inputs, Order::Canonical, Some(&shared), true))
    };
    ...
    assert!(batch(&mut rx[0], 22).contains(&Got::Vanish(1)));
}
