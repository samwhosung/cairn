...
/// Everything a tick changed in what is sent and what is saved, which replication and saving both
/// read. Entries come by row, and a row's in the order they happened.
...
pub struct Record {
    ...
    /// Each row's saved fields, as for `shown`, and `None` for a row that is gone.
...
/// The saved state of every row, kept in memory from each tick's record: the reader saving will
/// replace.
